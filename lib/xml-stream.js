...
function Decoder(encoding) {
  if (!Iconv.encodingExists(encoding)) {
    throw new Error(`Encoding format '${encoding}' isn't supported`);
  }
...
function XmlStream(stream, encoding) {
  ...
  this._encoder = makeDecoder(this._encoding);
...
function makeDecoder(encoding) {
  if (encoding && !/^utf-?8$/i.test(encoding)) {
    return new Decoder(encoding, 'utf8');
  }
  ...
  this._stream.on('data', function(data) {
    if (self._encoding) {
      ...
      if (/^\s*<[^>]+>/.test(prelude)) {
        ...
        self._encoder = makeDecoder(self._encoding);
